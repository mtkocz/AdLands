...
class MissileSystem {
  ...
  _createLockOnReticle() {
    ...
    this.lockOnReticle.style.cssText =
      "position:fixed;width:48px;height:48px;pointer-events:none;z-index:9999;" +
  ...
  _createAfterburnerSystem() {
    ...
    this._abPoints.layers.enable(1); // Bloom layer
  ...
  _emitAfterburner(missile) {
    ...
    for (let n = 0; n < count; n++) {
      ...
      ab.ages[i] = 0;
      ab.lifetimes[i] = 0.15 + Math.random() * 0.25; // 0.15-0.4s
      ab.sizes[i] = 0.5 + Math.random() * 1.0;
