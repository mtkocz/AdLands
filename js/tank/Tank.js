...
class Tank {
  ...
  checkTerrainCollision(prevTheta, prevPhi, deltaTime) {
    if (!this.planet?.terrainElevation && !this._terrainGrid) {
      ...
      if (this.state.phi < fallbackLimit || this.state.phi > Math.PI - fallbackLimit) {
        ...
        this.state.speed *= Math.pow(0.5, deltaTime * 10);
        return true;
      }
      return false;
    }
    ...
    if (!this._isTerrainBlocked(this.state.theta, prevPhi)) {
      ...
      this.state.phi = prevPhi;
      this.state.speed *= Math.pow(0.97, dtScaled);
      ...
      this.state.theta = prevTheta;
      this.state.phi = prevPhi;
      this.state.speed *= Math.pow(0.5, dtScaled);
    }
    return true;
  }
  ...
  _moveOnSphere(deltaTime) {
    ...
    // 0.97 slide = gentle scrape (retains 74% speed/sec), allows smooth wall-sliding
    // 0.5 block = firm stop when driving into a corner (stops in ~0.3s)
...
Tank.updateTankLOD = function (
  ...
  // LOD thresholds
  const LOD_DISTANCE = 100; // 0-100: detail, 100+: box
  const DOT_DISTANCE = 200; // 200+: dots (friendlies / all if commander)
