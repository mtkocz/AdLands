...
class TreadTracks {
  constructor(parentGroup, sphereRadius) {
    ...
    // 4 independent track spawners - one per corner of the tank
  ...
  registerTank(tankId, group, state) {
    this.trackedTanks.set(tankId, {
      ...
      spawners: {
        ...
        rearLeft: { sourcePuddle: null, lastSegmentPos: null, pickupPos: null },
        frontRight: {
          sourcePuddle: null,
          lastSegmentPos: null,
          pickupPos: null,
  ...
  update(
    ...
    for (const [tankId, tankData] of this.trackedTanks) {
      ...
      // Process each of the 4 independent spawners
      for (const [spawnerName, spawnerData] of Object.entries(spawners)) {
        ...
        if (puddle) {
          ...
          // Wheel is not touching oil - clear its state
  ...
  _addSegment(
    ...
    pool.segments[instanceIndex] = {
      ...
      cachedDistanceOpacity: 1.0,
    };

    // Advance ring buffer
    pool.nextIndex = (pool.nextIndex + 1) % pool.maxSegments;

    // Get current puddle opacity for initial color
    const opacity = sourcePuddle ? sourcePuddle.material.opacity : 1.0;
